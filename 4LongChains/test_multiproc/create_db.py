...
def extract_text_from_pdf_with_metadata(pdf_path, bibtex_citations):
    """Extract text and metadata from a given PDF using pdfplumber."""
...
def extract_text_from_txt(txt_path):
    """Extract text from a given text file."""
...
def validate_metadata(metadata):
    """Validate and possibly transform metadata to meet Chroma's requirements."""
    validated_metadata = {}
    for key, value in metadata.items():
        ...
        if isinstance(value, (str, int, float)):
            ...
            validated_metadata[key] = json.dumps(value)  # Serialize dict to JSON string
        else:
...
for pdf_file in pdf_files:
    ...
    all_texts_with_metadata.extend(extract_text_from_pdf_with_metadata(pdf_path, bibtex_citations))
